...
import type { Transaction, Category, Subscription } from "./api";
import type { TransactionType, CategoryType, BillingCycle } from "./base";
...
export function assertCategoryType(value: unknown): asserts value is CategoryType {
  if (!isCategoryType(value)) {
    throw new Error(`Invalid category type: ${value}`);
  }
}

export function assertBillingCycle(value: unknown): asserts value is BillingCycle {
...
export function assertSubscription(value: unknown): asserts value is Subscription {
