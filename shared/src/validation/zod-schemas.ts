...
const zodErrorMap: z.ZodErrorMap = (issue, ctx) => {
  ...
  if (issue.code === z.ZodIssueCode.too_small) {
    if (issue.type === 'string') {
      return { message: `${fieldName}は${issue.minimum}文字以上である必要があります` }
    }
    if (issue.type === 'number') {
      return { message: `${fieldName}は${issue.minimum}以上である必要があります` }
    }
  }
  
  if (issue.code === z.ZodIssueCode.too_big) {
    if (issue.type === 'string') {
      return { message: `${fieldName}は${issue.maximum}文字以下である必要があります` }
    }
    if (issue.type === 'number') {
      return { message: `${fieldName}は${issue.maximum}以下である必要があります` }
    }
  }
  
  // enum型のエラー
  if (issue.code === z.ZodIssueCode.invalid_enum_value) {
    return { message: `${fieldName}は${issue.options.join(', ')}のいずれかである必要があります` }
  }
  
  // カスタムエラー
  if (issue.code === z.ZodIssueCode.custom) {
    return { message: issue.message || ctx?.defaultError || 'バリデーションエラー' }
  }
...
export const transactionTypeSchema = z.literal('expense', {
  errorMap: () => ({ message: '取引種別は支出（expense）のみ許可されています' })
})
