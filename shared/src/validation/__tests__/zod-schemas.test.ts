  ...
  describe('エラーメッセージの日本語化', () => {
    it('必須フィールドのエラーメッセージが日本語になる', () => {
      ...
      if (!result.success) {
        ...
        const amountError = errors.find(e => e.path[0] === 'amount')
    ...
    it('範囲エラーのメッセージが日本語になる', () => {
      ...
      if (!result.success) {
        expect(result.error.errors[0].message).toContain('10000000以下である必要があります')
      }
    })
    ...
    it('enum型のエラーメッセージが日本語になる', () => {
      ...
      if (!result.success) {
        expect(result.error.errors[0].message).toContain('monthly, yearly, weeklyのいずれかである必要があります')
      }
    })
  })
})