...
#### テストヘルスモニタリング（2025年1月〜）

過剰なテストによる開発効率の低下を防ぐため、以下の観点でテストコードの健全性を継続的に監視：

**監視項目**:
1. **テストコード比率** - コンポーネントの複雑度に応じた適切なテスト量を維持
   - シンプル（100行未満）: 最大1.5倍
   - 中規模（100-300行）: 最大2倍
   - 大規模（300行以上）: 最大2.5倍

2. **テスト実行時間** - 開発効率に影響する遅いテストを検出
   - Frontend: 60秒以上で警告
   - API: 90秒以上で警告
...
**詳細**: [テストヘルスモニタリング](./docs/テスト/テストヘルスモニタリング.md)
