...
describe('useExpenses (Simplified)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // デフォルトで成功レスポンスを返す
    vi.mocked(transactionsApi.getExpenseTransactions).mockResolvedValue(mockExpenses);
  });
  ...
  describe('CRUD操作', () => {
    it('新規支出を作成できる', async () => {
      ...
      vi.mocked(transactionsApi.createTransaction).mockResolvedValue(newExpense);
      ...
      // 新規作成を実行
      let createdExpense: Transaction;
      await act(async () => {
        createdExpense = await result.current.createExpenseMutation({
          amount: 2000,
          description: '夕食',
          date: '2024-01-03',
          categoryId: '1',
    ...
    it('既存支出を更新できる', async () => {
      ...
      vi.mocked(transactionsApi.updateTransaction).mockResolvedValue(updatedExpense);
