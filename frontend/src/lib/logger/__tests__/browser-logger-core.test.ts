...
describe('Browser Logger Core', () => {
  ...
  let mockFetch: MockInstance<any[], Promise<Response>>;
  ...
  describe('ロガー初期化と設定', () => {
    ...
    it('環境に応じた適切な設定が適用される', () => {
      ...
      expect(devConfig.enableRemote).toBe(false);
  ...
  describe('基本ログ機能', () => {
    it('debug, info, warn, error メソッドが正しく動作する', async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ data: 'debug' })
      );
  ...
  describe('エラーハンドリングとフォールバック', () => {
    ...
    it('無効な設定に対してバリデーションが機能する', () => {
      ...
      expect(validateConfig(negativeBufferConfig)).toHaveProperty('bufferSize', 50);
      ...
      expect(validateConfig(zeroFlushConfig)).toHaveProperty('flushInterval', 5000);
    });
  });
});