...
describe("ExpenseStats", () => {
	...
	describe("基本的な表示", () => {
		test("統計データが正常に表示される", async () => {
			...
			expect(screen.getByTestId("balance-amount")).toHaveTextContent("¥24,691");
		});
		...
		test("主要カテゴリ情報が正常に表示される", () => {
			...
			// カテゴリ情報の確認
			expect(screen.getByTestId("top-expense-category")).toHaveTextContent("食費");
			expect(screen.getByTestId("top-expense-category")).toHaveTextContent("¥50,000");
			expect(screen.getByTestId("top-income-category")).toHaveTextContent("給与");
			expect(screen.getByTestId("top-income-category")).toHaveTextContent("¥100,000");
		});
		...
		test("期間比較情報が正常に表示される", () => {
			...
			// 前月比の数値確認
			expect(screen.getByTestId("monthly-comparison")).toHaveTextContent("+12.5%");
		});
	});
	...
	describe("空データ状態", () => {
		...
		test("空データでも基本的な構造は表示される", () => {
			...
			// カードは表示されているが、値が0で表示される
			expect(screen.getByTestId("monthly-balance-card")).toBeInTheDocument();
			expect(screen.getByTestId("total-income")).toHaveTextContent("¥0");
			expect(screen.getByTestId("total-expense")).toHaveTextContent("¥0");
	...
	describe("数値フォーマット", () => {
		test("日本円形式で正しくフォーマットされる", () => {
			...
			// カンマ区切りの確認
			expect(screen.getByTestId("total-income")).toHaveTextContent("¥1,234,567");
			expect(screen.getByTestId("total-expense")).toHaveTextContent("¥987,654");
			expect(screen.getByTestId("balance-amount")).toHaveTextContent("¥246,913");
		});
	...
	describe("アクセシビリティ", () => {
		test("適切なARIA属性が設定されている", () => {
			...
			// セクションにaria-labelが設定されている
			const statsContainer = screen.getByTestId("expense-stats");
			expect(statsContainer).toHaveAttribute("aria-label", "支出統計情報");

			// 各カードにrole属性が設定されている
			expect(screen.getByTestId("monthly-balance-card")).toHaveAttribute("role", "region");
			expect(screen.getByTestId("top-categories-card")).toHaveAttribute("role", "region");
			expect(screen.getByTestId("period-comparison-card")).toHaveAttribute("role", "region");
		});
	...
	describe("条件付きレンダリング", () => {
		...
		test("nullプロパティの安全な処理", () => {
			...
			expect(screen.getByTestId("top-expense-category")).toHaveTextContent("データなし");
			expect(screen.getByTestId("top-income-category")).toHaveTextContent("データなし");
		});
	});
