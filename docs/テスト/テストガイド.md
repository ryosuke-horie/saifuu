<!--
tags: #テスト #品質保証 #開発プロセス #テスト戦略 #ユニットテスト #E2Eテスト #コンポーネントテスト #TDD #品質基準 #Vitest #Playwright #React Testing Library #MSW #重要 #完了
category: 品質保証
priority: 高
status: 完了
last_updated: 2025-01-13
test_frameworks: ["Vitest", "Playwright", "React Testing Library"]
coverage_target: 80
-->
...
VRT(Visual Regression Testing)システムを削除し、以下の2層構造による品質担保に移行しました：
...
E2Eテスト (Playwright)          # 正常系のみ・最小限
    ↑
ユニットテスト (Vitest)          # 単体テスト・コンポーネント・エラー系
```

### 責務分担

- **Vitest**: ロジック・ユーティリティ関数・バリデーション・エラーハンドリング・コンポーネントテスト
...
カバレッジの数値目標よりも、以下の観点を重視します：

- **重要な機能の網羅**: ビジネスロジックやユーザーフローの核となる機能を確実にテスト
- **エッジケースの考慮**: 境界値や異常系の振る舞いを適切にカバー
- **リグレッション防止**: 過去のバグが再発しないことを保証
- **保守性の確保**: テストコード自体が理解しやすく、メンテナンスしやすいこと
