...
describe('logger utilities', () => {
	...
	describe('LogContext', () => {
		it('ログコンテキストから適切な操作タイプを判定する', () => {
			const readContext: LogContext = {
				resource: 'transactions',
				operation: 'findMany',
			}

			const writeContext: LogContext = {
				resource: 'subscriptions',
				operation: 'create',
			}

			const deleteContext: LogContext = {
				resource: 'categories',
				operation: 'delete',
			}

			// 実装時にヘルパー関数が追加されることを想定
			expect(readContext.operation).toContain('find')
			expect(writeContext.operation).toBe('create')
			expect(deleteContext.operation).toBe('delete')
		})
	})
